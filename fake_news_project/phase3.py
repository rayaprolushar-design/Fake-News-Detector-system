...
def main():
    ...
    # Train Models
    lr_model, lr_preds, lr_acc = train_logistic_regression(X_train, y_train, X_test, y_test)
    print(f"Logistic Regression accuracy: {lr_acc*100:.2f}%\n")

    nb_model, nb_preds, nb_acc = train_naive_bayes(X_train, y_train, X_test, y_test)
    ...
    # Predictions
    print("\n--- Try it out! ---")
    predict_news("Scientists confirm new vaccine is 95% effective", lr_model, tfidf)
    predict_news("SHOCKING: Government hiding alien contact since 1947!!", lr_model, tfidf)
    predict_news("Federal Reserve holds interest rates steady", lr_model, tfidf)
